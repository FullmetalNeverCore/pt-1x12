...
class Radio(commands.Cog):
    ...
    def __init__(self,bot):
        ...
        self.atomic_playlist = {'Atomic Heart - Трава у дома' : 'https://www.youtube.com/watch?v=gKVXaMeazAQ',
                                ...
                                'Atomic Heart - Arlekino (feat. Alex Terrible)' : "https://www.youtube.com/watch?v=YVar_DNBlu4"}
        self.YDL_OPTIONS = {
    ...
    async def is_playing(self):
         if not self.voice_client is None:
            if self.voice_client.is_playing():
                ...
                if self.atomic:
                     self.was_atomic = not self.was_atomic
                     self.atomic = False
    ...
    async def update_status(self):
        ...
        if title != self.previous and not self.atomic or self.dev_test:
             ...
             if random.randint(0,100) <= self.chance and not self.was_atomic:
                  if self.voice_client is not None and self.voice_client.is_playing():await self.atomic_heart()
             #else:
                  ##await self.init_radio('1',None)
        if not self.atomic:
